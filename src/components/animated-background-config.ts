...
export const getKeyboardState = ({
  ...
  // Use a more stable scale offset that doesn't shrink the keyboard too much on mobile
  // cap it between 0.8 and 1.5 to prevent extreme scaling
  const pixelRatio = typeof window !== 'undefined' ? window.devicePixelRatio : 1;
  const scaleOffset = Math.max(0.8, Math.min(1.5, +(pixelRatio - 0.4).toFixed(1)));
