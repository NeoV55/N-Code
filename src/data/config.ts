const config = {
  title: "Navinder | Full-Stack Web3 Security Architect, AI Researcher & Systems Engineer",
  siteName: "Navinder",
  description: {
    long: "Explore the portfolio of Navinder, a cross-domain technologist specializing in cybersecurity, blockchain architecture, and AI-driven automation. From building secure DeFi protocols and high-performance GPU compute farms to developing ML-powered trading engines, I bridge the gap between hardware, security, and scalable software.",
    short:
      "Navinder: A security-first engineer specializing in Blockchain, AI, and Full-Stack Systems.",
  },
  keywords: [
    "Navinder",
    "Cybersecurity Engineer",
    "Blockchain Developer",
    "AI Research",
    "Smart Contract Audit",
    "Penetration Testing",
    "Web3 Security",
    "GPU Optimization",
    "Solidity",
    "Rust",
    "Python AI",
    "DeFi Architect",
    "Full-Stack Developer",
    "Infrastructure Automation",
    "Ethical Hacking",
    "MLOps",
    "Web3 Security Architect",
    "AI Systems Engineer",
    "AI Agent Developer",
    "Blockchain Security",
    "MLOps",
    "GPU Infrastructure",
    "Smart Contract Auditor",
    "Full-Stack Web3",
    "DeFi Security",
    "Cybersecurity AI",
    "Ethical Hacking",
    "Solidity Rust AI",
    "Systems Optimization",
  ],
  
  author: "Navinder",
  email: "navinder.tech@gmail.com",
  site: "https://navinder.tech",

  // for github stars button
  githubUsername: "NeoV55",


   get ogImg() {
    return this.site + "/image.png";
  },
  social: {
    twitter: "https://x.com/Navinder_05",
    linkedin: "https://my.linkedin.com/in/navinder-tech-b2600538a",
    facebook: "https://www.facebook.com/navinder.ram.7",
    medium : "https://medium.com/@ndram.com",
    substack : "https://neo555.substack.com/",
    devdotto : "https://dev.to/navinder",
    github: "https://github.com/NeoV55",
  },
};
export { config };
